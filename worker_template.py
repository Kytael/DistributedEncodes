            ...
            if proc.poll() is None:
                try:
                    if platform.system() == 'Windows':
                        if suspend: suspend_windows_process(proc.pid)
                        else: resume_windows_process(proc.pid)
                    else:
...
def worker_task(worker_id, manager_url, temp_dir, single_mode=False):
    ...
    while not SHUTDOWN_EVENT.is_set():
        ...
        try:
            ...
            try: r = requests.get(f"{manager_url}/get_job", timeout=10)
            ...
            if data and data.get("status") == "ok":
                ...
                if proc.returncode == 0 and os.path.exists(local_dst):
                    ...
                    with ProgressFileReader(local_dst, upload_cb) as f:
                        requests.post(f"{manager_url}/upload_result", files={'file': (job_id, f)}, data={'job_id': job_id, 'worker_id': worker_id, 'duration': total_min})
