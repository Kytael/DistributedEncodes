...
def init_db():
    with db_lock:
        # [FIX] Set timeout to 60s to prevent locking errors
        conn = sqlite3.connect(DB_FILE, timeout=60)
        
        # [FIX] Enable Write-Ahead Logging (WAL) for better concurrency
...
def admin_action():
    ...
    with db_lock:
        ...
        try:
            ...
            # [NEW] Clear Stale Workers (Prune)
            elif action == 'clear_stale':
                # 10 minute cutoff for manual prune
                    ...
                    for row in cursor.fetchall():
                        ...
                        if last_up:
                            try:
                                ...
                                if (now - l_time).total_seconds() > 7200:
                                    logs_to_write.append(("WARN", f"Worker {worker_id} timed out (No Heartbeat). Resetting.", jid))
